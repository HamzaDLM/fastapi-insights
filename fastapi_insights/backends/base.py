...
class _BaseStore(ABC):
    ...
    def _get_bucket_size(
        ...
        ideal_bucket_size = max(
            self.bucket_sizes[0], time_range_seconds // target_points
        )

        suitable_buckets = [bs for bs in self.bucket_sizes if bs >= ideal_bucket_size]
        ...
        for bucket_size in sorted(self.bucket_sizes, reverse=True):
...
class MetricsStore(_BaseStore):
    ...
    def get_metrics(
        ...
        return {
            ...
            "system_metrics": self.get_system_metrics_series(
                bucket_size, ts_from, ts_to
            )
            | {
...
class AsyncMetricsStore(_BaseStore):
    ...
    async def _get_read_write_series(
        self, bucket_size: int, ts_from: int, ts_to: int
    ) -> list:
        grouped = defaultdict(list)
    ...
    async def _get_top_slowest_routes(
        self, bucket_size: int, ts_from: int, ts_to: int, count: int = 5
    ) -> dict:
        latency_averages = defaultdict(float)
    ...
    async def get_metrics(
        ...
        return {
            ...
            "read_write": await self._get_read_write_series(
                bucket_size, ts_from, ts_to
            ),
            "status_code": await self._get_status_code_series(
                bucket_size, ts_from, ts_to
            ),
            "top_routes": await self._get_top_routes(bucket_size, ts_from, ts_to),
            "requests_per_method": await self._get_requests_per_method(
                bucket_size, ts_from, ts_to
            ),
            "top_slowest_routes": await self._get_top_slowest_routes(
                bucket_size, ts_from, ts_to
            ),
            "top_error_prone_requests": await self._get_top_error_prone_requests(
                bucket_size, ts_from, ts_to
            ),
            "system_metrics": await self.get_system_metrics_series(
                bucket_size, ts_from, ts_to
            )
            | {
                "num_threads": psutil.cpu_count(logical=True),
